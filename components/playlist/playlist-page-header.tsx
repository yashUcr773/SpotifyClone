...
export default function PlaylistPageHeader({ playlist }: PlaylistPageHeaderProps) {
    ...
    return (
        <div onClick={() => { openModal('editPlaylist', { playlist }) }} className="min-h-[144px] relative  px-4 py-2 mb-2 flex flex-row gap-x-4 mt-16 group  cursor-pointer max-w-2xl" >
            ...
            <div className="flex flex-col gap-y-4 justify-end">
                ...
                <div className="hidden group-hover:flex items-center gap-x-2 absolute p-1 top-0 right-0">
                    ...
                    <TooltipWrapper label="Delete">
                        <Trash size={24} onClick={() => { openModal('editPlaylist', { playlist }) }} className="cursor-pointer ml-auto  text-white transition"></Trash>
                    </TooltipWrapper>
                </div>
            </div>
        </div>
